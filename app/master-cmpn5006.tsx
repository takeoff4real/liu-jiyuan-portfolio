...
export function MasterCmpn5006({ onBack }: { onBack: () => void }) {
  ...
  return <section className="detail-page master-detail master-cmpn5006">
    ...
    <section className="recording-media" aria-labelledby="recording-media-title"><div className="section-intro"><p className="kicker">LISTEN</p><h2 id="recording-media-title">第一次作品音轨</h2><p className="section-description">这是一条以室内日常为线索的声音作品，展示我如何从录音提案出发，组织空间、声音与听觉节奏。</p></div><div className="recording-media-grid"><article><div className="recording-track-head"><p className="kicker">FIRST TRACK</p><span className="recording-track-index">01 / WAV</span></div><h3>第一次作品音轨</h3><p className="recording-track-note">原始提交音轨 · 点击播放</p><div className="recording-player"><audio controls preload="metadata" src="/assets/masters/recording-portfolio/a2-track.wav">你的浏览器不支持音频播放。</audio></div></article></div></section>
    <section className="arin-submissions cmpn5006-submissions" aria-labelledby="cmpn5006-submissions-title"><div className="section-intro"><p className="kicker">SUBMISSIONS / 03</p><h2 id="cmpn5006-submissions-title">课程作业与原始提交</h2><p className="section-description">选择一个提交阶段，查看我在提案、录音、反思与最终作品集中的工作方式。</p></div><div className="arin-submission-layout"><nav className="arin-submission-nav" aria-label="CMPN5006 作业导航">{submissions.map((item, index) => <button key={item.number} className={active === index ? 'is-active' : ''} onClick={() => setActive(index)}><span>{item.number}</span><span>{item.label}</span></button>)}</nav><article className="arin-submission-detail"><p className="kicker">{current.label}</p><h3>{current.title}</h3><p>{current.body}</p><a className="master-report-link is-static" href={current.file} target="_blank" rel="noreferrer">{current.fileLabel}</a></article></div>{activeSlides.length > 0 && <SlideCarousel slides={activeSlides} className="cmpn5006-slide-stack" />}</section>
