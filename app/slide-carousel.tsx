...
export function SlideCarousel({ slides, className = '', visibleCount = 2, layout = 'carousel' }: { slides: Slide[]; className?: string; visibleCount?: 2 | 4 | 5; layout?: 'carousel' | 'masonry' }) {
  ...
  return <div className={`slide-carousel slide-carousel--idea9202-style ${layout === 'masonry' ? 'slide-carousel--masonry' : ''} ${className}`} style={{ '--slide-visible-count': visibleCount } as React.CSSProperties} aria-label={layout === 'masonry' ? '瀑布流作品浏览' : '横向作品浏览'} aria-roledescription="carousel">
    <div className="slide-carousel-viewport">
      <div className="slide-carousel-track" style={layout === 'masonry' ? undefined : { transform: `translateX(calc(-${active} * (var(--slide-item-width) + 18px)))` }}>
    ...
    {layout !== 'masonry' && slides.length > visibleCount && <div className="slide-carousel-controls">
