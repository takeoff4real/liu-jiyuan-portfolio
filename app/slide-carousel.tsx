...
export function SlideCarousel({ slides, className = '', visibleCount = 2 }: { slides: Slide[]; className?: string; visibleCount?: 2 | 4 | 5 }) {
  ...
  return <div className={`slide-carousel slide-carousel--idea9202-style ${className}`} style={{ '--slide-visible-count': visibleCount } as React.CSSProperties} aria-label="横向作品浏览">
    ...
    {lightbox !== null && <div className="lightbox" role="dialog" aria-modal="true" aria-label="全屏作品查看" onClick={() => setLightbox(null)}><button type="button" className="close-lightbox" onClick={() => setLightbox(null)} aria-label="关闭全屏查看">×</button><button type="button" className="lightbox-nav lightbox-nav-prev" onClick={(event) => { event.stopPropagation(); setLightbox((value) => value === null ? slides.length - 1 : (value - 1 + slides.length) % slides.length) }} aria-label="上一张">←</button><img src={slides[lightbox].src} alt={slides[lightbox].alt} onClick={(event) => event.stopPropagation()} /><button type="button" className="lightbox-nav lightbox-nav-next" onClick={(event) => { event.stopPropagation(); setLightbox((value) => value === null ? 0 : (value + 1) % slides.length) }} aria-label="下一张">→</button><div className="lightbox-caption">{String(lightbox + 1).padStart(2, '0')} / {String(slides.length).padStart(2, '0')} · {slides[lightbox].alt}<span>← → 切换　ESC 关闭</span></div></div>}
  </div>
}
